...
RUN_ID=${RUN_ID:-task23_v153_${STAMP}}
OUT_ROOT=${OUT_ROOT:-${OUT_BASE}/${RUN_ID}}
SESSION=${SESSION:-task23_v153_${STAMP}}
JOB_NAME=${JOB_NAME:-task23v153_${STAMP}}
PORT=${PORT:-9723}
...
mkdir -p "${OUT_ROOT}"
cp -p "${ROOT}/run_task23_v153.sh" "${ROOT}/inputs.env.example" \
...
tmux -f /dev/null -L hlei573borrow new-session -d -s "${SESSION}" \
  "bash -lc 'set -o pipefail; srun -p acd_u --gres=gpu:2 -c8 --mem=${MEM_MB}M --time=02:00:00 --job-name=${JOB_NAME} ${EXCLUDE_ARG[*]} bash -lc \"cd ${ROOT} && INPUTS_FILE=${INPUTS_FILE} RUN_ID=${RUN_ID} OUT_ROOT=${OUT_ROOT} PORT=${PORT} bash ${ROOT}/run_task23_v153.sh\" 2>&1 | tee -a ${OUT_ROOT}/submit.log; rc=\\\${PIPESTATUS[0]}; echo [TMUX_EXIT] status=\\\${rc}; exec bash'"
